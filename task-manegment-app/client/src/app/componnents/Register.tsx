...
export default function Register() {
  ...
  const [signUpMutation, { data, loading: Apolloloading, error: ApolloErorr }] =
    useMutation(SIGN_UP_MUTATION);

  const handleSubmit = async (e: any) => {
    console.log(email, password, name, 'handleSubmit');
    e.preventDefault();
    setError('');
    try {
      setLoading(true);
      ...
      if (ApolloErorr) {
        console.log(`Submission error! ${ApolloErorr}`);
      }
    ...
    <div className="flex min-h-screen items-center justify-center bg-indigo-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-96">
        ...
        <form className="mt-6 space-y-6" action="#" method="POST">
          ...
          <div>
            ...
            <div className="mt-1">
              <input
                ...
                type="password" // Fix the input type here
                autoComplete="current-password"
                required
                className="w-full px-4 py-2 text-gray-900 bg-gray-100 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
              />
          ...
          <div>
            {error && (
              <p id="email-error" className="text-red-600 text-center">
                {error}
              </p>
            )}
          </div>
